...
export class DoctorRepositoryImpl implements DoctorRepository {
  ...
  CrearDoctor(Doctor: DoctorEntity): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.DoctorImplements.save(NuevoProducto)
        .then(() => {
          resolve();
        })
        .catch((error) => {
  ...
  EditarHistorialMascota(Doctor: DoctorEntity): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.findById(doctor)
        .then((doctor) => {
          if (!doctor) {
            throw new Error(`No se encuentra el doctor con el Id: ${Doctor._id}`);
          }

          doctor.PacienteId = Doctor.PacienteId;

          this.DoctorImplements.save(doctor)
            .then(() => {
              resolve();
            })
            .catch((error) => {
  ...
  VerTodosDoctores(): Promise<DoctorEntity[]> {
    return new Promise((resolve, reject) => {
